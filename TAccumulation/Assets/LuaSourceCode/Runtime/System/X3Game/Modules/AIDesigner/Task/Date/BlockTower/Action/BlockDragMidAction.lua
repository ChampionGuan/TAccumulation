...
---中间块的拿行为
---@class BlockDragMidAction:AIAction
...
local BlockDragMidAction = class("BlockDragMidAction", AIAction)

---
function BlockDragMidAction:OnEnter()
    ...
    local moveDirection = rotation * Vector3.right
    moveDirection.y = 0
...
---@return AITaskState
function BlockDragMidAction:OnUpdate()
    if self.gameData.selectedBlock == nil then
        return AITaskState.Failure
    end
    if self.gameData.blockTowerControlMode == Const.ControlMode.SelectedMid and self.runningTime < self.dragTime then
