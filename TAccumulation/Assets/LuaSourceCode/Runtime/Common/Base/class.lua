...
---@param super table
---@param isBaseClass boolean
---@param isPoolEnable boolean 是否开启缓存池
---@return table
function class(classname, super, isBaseClass,isPoolEnable)
    ...
    if superType == "function" or (super and super.__ctype == 1) then
        ...
        cls.__index = cls
        if isBaseClass then
            cls[string.concat("__",classname)] = true
        end
        if isPoolEnable then
            cls.__pool__ = {}
            function cls.GetFromPool(...)
                local ret
                if #cls.__pool__>0 then
                    ret = table.remove(cls.__pool__)
                else
                    ret = setmetatable({}, cls)
                    ret:ctor(...)
                end
                if ret.Init then
                    ret:Init(...)
            ...
            function cls.ReleaseToPool(ret)
                if ret then
                    if not table.containsvalue(cls.__pool__,ret) then
                        table.insert(cls.__pool__,ret)
                        if ret.Destroy then
                            ret:Destroy()
                        end
                        table.clear(ret)
            ...
            function cls.ClearAll()
                table.clear(cls.__pool__)
            end
        end

        function cls.new(...)
            if isPoolEnable then
                return cls.GetFromPool(...)
            end
...
function xstruct(structname, super)
    ...
    if super then
        ...
        cls = { ctor = emptyCtor }
        cls.__index = function(t, keyName)
            return rawget(cls, keyName)
        end
