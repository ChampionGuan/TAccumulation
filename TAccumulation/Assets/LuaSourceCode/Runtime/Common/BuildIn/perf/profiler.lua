...
local function report(sort_by)
    ...
    local header = get_title()
    local stat_interval = get_time() - (start_time or get_time())
    
    local report_list = {}
    for _, rp in pairs(func_info_map) do
        if rp.total_time >0 and rp.db_info.short_src and rp.db_info.short_src~="" and rp.db_info.linedefined and rp.db_info.linedefined>0 then
            table.insert(report_list, {
                total_time = rp.total_time,
                count = rp.count,
                average = rp.total_time / rp.count,
                name = rp.db_info.name or "[anonymous]",
