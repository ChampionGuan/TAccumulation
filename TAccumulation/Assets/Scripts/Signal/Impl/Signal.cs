...
public delegate void Action<T, U, V, W, X>(T obj1, U obj2, V obj3, W obj4, X obj5);
...
/// Base concrete form for a Signal with four parameters
public class Signal<T, U, V, W, X> : BaseSignal
{
    public event Action<T, U, V, W, X> Listener = delegate { };
    public event Action<T, U, V, W, X> OnceListener = delegate { };

    public void AddListener(Action<T, U, V, W, X> callback)
    {
        Listener = this.AddUnique(Listener, callback);
    }

    public void AddOnce(Action<T, U, V, W, X> callback)
    {
        OnceListener = this.AddUnique(OnceListener, callback);
    }

    public void RemoveListener(Action<T, U, V, W, X> callback) { Listener -= callback; }
    public override List<Type> GetTypes()
    {
        List<Type> retv = new List<Type>();
        retv.Add(typeof(T));
    ...
    public void Dispatch(T type1, U type2, V type3, W type4, X type5)
    {
    ...
    private Action<T, U, V, W, X> AddUnique(Action<T, U, V, W, X> listeners, Action<T, U, V, W, X> callback)
    {
        if (!listeners.GetInvocationList().Contains(callback))
        {
            listeners += callback;
