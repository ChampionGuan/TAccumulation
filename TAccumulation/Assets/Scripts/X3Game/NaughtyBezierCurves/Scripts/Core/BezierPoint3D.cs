...
public class BezierPoint3D : MonoBehaviour
{
    ...
    // Serializable Fields
    [SerializeField] [Tooltip("The curve that the point belongs to")]
    private BezierCurve3D curve = null;

    [SerializeField] private HandleType handleType = HandleType.Connected;

    [SerializeField] private Vector3 leftHandleLocalPosition = new Vector3(-0.5f, 0f, 0f);

    [SerializeField] private Vector3 rightHandleLocalPosition = new Vector3(0.5f, 0f, 0f);

    [SerializeField] private Vector3 moveVector = new Vector3(0f, 0.5f, 0f);
    // Properties
    ...
    {
        get { return this.handleType; }
        set { this.handleType = value; }
    }
    ...
    {
        get { return this.transform.TransformPoint(this.LeftHandleLocalPosition); }
        set { this.LeftHandleLocalPosition = this.transform.InverseTransformPoint(value); }
    }
    ...
    {
        get { return this.transform.TransformPoint(this.RightHandleLocalPosition); }
        set { this.RightHandleLocalPosition = this.transform.InverseTransformPoint(value); }
    }

    /// <summary>
    /// Add one Vector3 for describe move Direction.
    /// </summary>
    public Vector3 MoveVector
    {
        get { return this.transform.TransformPoint(this.moveVector); }
        set
        {
            // this.moveVector = Vector3.Normalize(this.transform.InverseTransformPoint(value)) * 0.5f;
            this.moveVector = this.transform.InverseTransformPoint(value);
        }
    }

    public Vector3 RawMoveVector
    {
    ...
    public void Awake()
    {
        sourcePostion = this.transform.localPosition;
    }
}