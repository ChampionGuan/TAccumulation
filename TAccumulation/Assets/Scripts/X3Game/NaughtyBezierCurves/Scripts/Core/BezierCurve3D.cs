...
public class BezierCurve3D : MonoBehaviour
{
    ...
    [SerializeField] [Range(0f, 1f)] float normalizedTime = 0.5f;

    // Properties        
    public int Sampling
    {
        get { return this.sampling; }
        set { this.sampling = value; }
    }
    ...
    {
        get { return this.KeyPoints.Count; }
    }
    ...
    {
        BezierPoint3D newPoint = new GameObject("Point " + this.KeyPoints.Count, typeof(BezierPoint3D))
        ...
        else
        {
            if (index == 0)
            {
                newPoint.Position = (this.KeyPoints[0].Position - this.KeyPoints[1].Position).normalized +
                                    this.KeyPoints[0].Position;
            }
            else if (index == this.KeyPointsCount)
            {
                newPoint.Position =
                    (this.KeyPoints[index - 1].Position - this.KeyPoints[index - 2].Position).normalized +
                    this.KeyPoints[index - 1].Position;
            }
            else
            {
                newPoint.Position =
                    BezierCurve3D.GetPointOnCubicCurve(0.5f, this.KeyPoints[index - 1], this.KeyPoints[index]);
            }
        }
    ...
    public Vector3 GetPoint(float time, bool isLocalPosition = false)
    {
        ...
        return BezierCurve3D.GetPointOnCubicCurve(timeRelativeToSegment, startPoint, endPoint, isLocalPosition);
    }
    ...
    public float GetApproximateLength()
    {
        ...
        {
            length += BezierCurve3D.GetApproximateLengthOfCubicCurve(this.KeyPoints[i], this.KeyPoints[i + 1],
                subCurveSampling);
        }
    ...
    public void GetCubicSegment(float time, out BezierPoint3D startPoint, out BezierPoint3D endPoint,
        out float timeRelativeToSegment)
    {
        ...
        {
            subCurvePercent =
                BezierCurve3D.GetApproximateLengthOfCubicCurve(this.KeyPoints[i], this.KeyPoints[i + 1],
                    subCurveSampling) / approximateLength;
    ...
    public static Vector3 GetPointOnCubicCurve(float time, BezierPoint3D startPoint, BezierPoint3D endPoint, bool isLocalPosition = false)
    {
        if (isLocalPosition)
        {
            return GetPointOnCubicCurve(time, startPoint.LocalPosition, endPoint.LocalPosition, 
                startPoint.LocalPosition + startPoint.RightHandleLocalPosition,
                endPoint.LocalPosition + endPoint.LeftHandleLocalPosition);
        }
        else
        {
            return GetPointOnCubicCurve(time, startPoint.Position, endPoint.Position, startPoint.RightHandlePosition,
                endPoint.LeftHandlePosition);    
        }
    }

    public static Vector3 GetPointOnCubicCurve(float time, Vector3 startPosition, Vector3 endPosition,
    ...
    public static Quaternion GetRotationOnCubicCurve(float time, Vector3 up, BezierPoint3D startPoint,
        BezierPoint3D endPoint)
    {
        return GetRotationOnCubicCurve(time, up, startPoint.Position, endPoint.Position, startPoint.RightHandlePosition,
            endPoint.LeftHandlePosition);
    }

    public static Quaternion GetRotationOnCubicCurve(float time, Vector3 up, Vector3 startPosition, Vector3 endPosition,
        Vector3 startTangent, Vector3 endTangent)
    {
    ...
    {
        return GetTangentOnCubicCurve(time, startPoint.Position, endPoint.Position, startPoint.RightHandlePosition,
            endPoint.LeftHandlePosition);
    }

    public static Vector3 GetTangentOnCubicCurve(float time, Vector3 startPosition, Vector3 endPosition,
        Vector3 startTangent, Vector3 endTangent)
    {
        float t = time;
        float u = 1f - t;
    ...
    public static Vector3 GetBinormalOnCubicCurve(float time, Vector3 up, BezierPoint3D startPoint,
        BezierPoint3D endPoint)
    {
        return GetBinormalOnCubicCurve(time, up, startPoint.Position, endPoint.Position, startPoint.RightHandlePosition,
            endPoint.LeftHandlePosition);
    }

    public static Vector3 GetBinormalOnCubicCurve(float time, Vector3 up, Vector3 startPosition, Vector3 endPosition,
        Vector3 startTangent, Vector3 endTangent)
    {
        Vector3 tangent = GetTangentOnCubicCurve(time, startPosition, endPosition, startTangent, endTangent);
    ...
    public static Vector3 GetNormalOnCubicCurve(float time, Vector3 up, BezierPoint3D startPoint,
        BezierPoint3D endPoint)
    {
        return GetNormalOnCubicCurve(time, up, startPoint.Position, endPoint.Position, startPoint.RightHandlePosition,
            endPoint.LeftHandlePosition);
    }

    public static Vector3 GetNormalOnCubicCurve(float time, Vector3 up, Vector3 startPosition, Vector3 endPosition,
        Vector3 startTangent, Vector3 endTangent)
    {
        Vector3 tangent = GetTangentOnCubicCurve(time, startPosition, endPosition, startTangent, endTangent);
    ...
    {
        return GetApproximateLengthOfCubicCurve(startPoint.Position, endPoint.Position, startPoint.RightHandlePosition,
            endPoint.LeftHandlePosition, sampling);
    }

    public static float GetApproximateLengthOfCubicCurve(Vector3 startPosition, Vector3 endPosition,
        Vector3 startTangent, Vector3 endTangent, int sampling)
    {
        float length = 0f;
    ...
    /// <summary>
    /// 获取贝塞尔曲线时间参数t时刻的全部采样节点Position
    /// </summary>
    public List<Vector3> GetCurvePoints()
    {
        List<Vector3> resultPointsList = new List<Vector3>();
        if (this.KeyPointsCount > 1)
        {
            // Get the curve Points
            Vector3 fromPoint = this.GetPoint(0f);
            resultPointsList.Add(fromPoint);
            for (int i = 0; i < this.Sampling; i++)
            {
                float time = (i + 1) / (float)this.Sampling;
                Vector3 toPoint = this.GetPoint(time);
                resultPointsList.Add(toPoint);
            }
        }
    ...
    public List<Vector3> GetCurvePoints_LocalPosition()
    {
        List<Vector3> resultPointsList = new List<Vector3>();
        if (this.KeyPointsCount > 1)
        {
            Vector3 fromPoint = this.GetPoint(0f, true);
            resultPointsList.Add(fromPoint);
            for (int i = 0; i < this.Sampling; i++)
            {
                float time = (i + 1) / (float)this.Sampling;
                Vector3 toPoint = this.GetPoint(time, true);
                resultPointsList.Add(toPoint);
            }
        }
    ...
    /// <summary>
    /// 获取贝塞尔曲线全部控制节点Position
    /// </summary>
    public List<Vector3> GetCurveControlPoints()
    {
        List<Vector3> drawLinesPostions = new List<Vector3>();
        if (keyPoints.Count > 0)
        {
            for (int i = 0; i < keyPoints.Count; i++)
            {
                var node = keyPoints[i];
                if (i != 0) drawLinesPostions.Add(node.LeftHandleLocalPosition + node.LocalPosition);
                drawLinesPostions.Add(node.LocalPosition);
                if (i != keyPoints.Count - 1)
                    drawLinesPostions.Add(node.RightHandleLocalPosition + node.LocalPosition);
            }
        }
        return drawLinesPostions;
    }
    

    public List<Vector3> GetCurvePointsOffset(int idx)
    {
        ...
        resultPointsList.AddRange(remain);
        return resultPointsList;
    }

    // 获取当前Time的计算节点Idx
    public int GetCurPointIdxThisTime(float time)
    {
        return Mathf.RoundToInt(this.Sampling * Mathf.Abs(time) + 1);
    }
    
    // 获取当前Time的Bezier节点次序Idx
    public int GetCurBezierPointIdxThisTime(float time)
    {
        return Mathf.RoundToInt(this.KeyPointsCount * Mathf.Abs(time) + 1);
    }
}