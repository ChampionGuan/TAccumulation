...
public class BezierPoint3DEditor : UnityEditor.Editor
{
    ...
    public static float arrowCapSize = 0.9f;
    ...
    protected virtual void OnEnable()
    {
        ...
        this.moveVector = this.serializedObject.FindProperty("moveVector");
    }
    ...
    public override void OnInspectorGUI()
    {
        ...
        EditorGUI.BeginChangeCheck();
        EditorGUILayout.PropertyField(this.moveVector);
    ...
    public static void DrawPointSceneGUI(BezierPoint3D point, Handles.CapFunction drawPointFunc,
        Handles.CapFunction drawHandleFunc)
    {
        ...
        Vector3 newPointPosition = Handles.FreeMoveHandle(point.Position, point.transform.rotation,
            HandleUtility.GetHandleSize(point.Position) * BezierPoint3DEditor.pointCapSize, Vector3.one * 0.5f,
            drawPointFunc);
        ...
        Vector3 newLeftHandlePosition = Handles.FreeMoveHandle(point.LeftHandlePosition, point.transform.rotation,
            HandleUtility.GetHandleSize(point.LeftHandlePosition) * BezierPoint3DEditor.handleCapSize, Vector3.zero,
            drawHandleFunc);
        ...
        Vector3 newRightHandlePosition = Handles.FreeMoveHandle(point.RightHandlePosition, point.transform.rotation,
            HandleUtility.GetHandleSize(point.RightHandlePosition) * BezierPoint3DEditor.handleCapSize, Vector3.zero,
            drawHandleFunc);
        ...
        Handles.DrawLine(point.Position, point.MoveVector);
        Vector3 newMoveVector = Handles.FreeMoveHandle(point.MoveVector, point.transform.rotation,
            HandleUtility.GetHandleSize(point.MoveVector) * BezierPoint3DEditor.arrowCapSize, Vector3.one,
            Handles.ArrowHandleCap);

        if (point.MoveVector != newMoveVector)
        {
            Undo.RegisterCompleteObjectUndo(point, "Move Direction Vector");
            point.MoveVector = newMoveVector;
        }
    }
