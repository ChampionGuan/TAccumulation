...
public class TrackBindCtrl : MonoBehaviour
{
    ...
    public void PlayNewTrack(TimelineExtInfo timelineExtInfo, AnimationClip animClip)
    {
        ...
        Init();
        PapeGames.X3.X3Debug.Log("lta test Start to Bind Track: " + timelineExtInfo.name);
        var weight = 1f / (float)(_bindedTrackDic.Count + 1);
        int inputPort = 0;
        foreach (var VARIABLE in _bindedTrackDic)
        {
            _unifiedTrackMixer.SetInputWeight(VARIABLE.Value, weight);
            _unifiedTrackMixer.ConnectInput(_workingInputPortDic[VARIABLE.Key], VARIABLE.Value, 0);
        }
    ...
    public void StopNewTrack(TimelineExtInfo timelineExtInfo)
    {
        ...
        PapeGames.X3.X3Debug.Log("lta test Start to Unbind Track: " + timelineExtInfo.name);
