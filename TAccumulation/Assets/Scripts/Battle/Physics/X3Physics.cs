...
namespace X3Battle
{
    public struct CircleData
    {
        ...
        public CircleData(Vector3 pos, float radius, CqCollider collider)
        {
            this.pos = pos;
            this.radius = radius;
    ...
    public static partial class X3Physics
    {
        ...
        private static List<CqCollider> _tempColliders;
        private static List<CircleData> _tempCircles;
        private static CqRaycastHit[] _raycastHit;
        private static BoundingShape _tempCubeShape = new BoundingShape();
        public static bool debugModel;

        private static CollisionQuery.CqQueryScene collisionQueryScene;
        public static CollisionQuery.CqQueryScene Collision => collisionQueryScene;
        ...
        public static void TryInit()
        {
            if (_tempColliders != null)
                return;
            _tempColliders = new List<CqCollider>(MaxResultNum);
            _tempCircles = new List<CircleData>(MaxResultNum);
            _raycastHit = new CqRaycastHit[MaxResultNum];
            debugModel = false;
            _Init();

            collisionQueryScene = new CollisionQuery.CqQueryScene();
        }
        ...
        /// <returns></returns>
        public static int CollisionTest(Vector3 centPos, Vector3 prevCenterPos, Vector3 rot, BoundingShape shape, bool isContinuousMode, ref CqCollider[] colliders, int layerMask)
        {
#if UNITY_EDITOR
        ...
        private static int CollisionTest(Vector3 centerPos, Quaternion rot, BoundingShape shape, CqCollider[] result, int layerMask)
        {
        ...
        private static int CollisionTest(ShapeType type, Vector3 centerPos, Quaternion rot, BoundingShape shape, CqCollider[] result,int layerMask)
        {
            ...
            switch (type)
            {
                case ShapeType.Cube:
                    ...
                    hitNum = Collision.BoxOverlap(new CqBox(new RigidTransform(rot, centerPos), half), result, layerMask);
                    break;
                case ShapeType.Sphere:
                    //hitNum = Physics.OverlapSphereNonAlloc(centerPos, shape.Radius, result, layerMask, flag);
                    hitNum = Collision.SphereOverlap(new CqSphere(centerPos, shape.Radius), result,layerMask);
                    break;
                case ShapeType.FanColumn: 
                    ...
                    hitNum = Collision.CapsuleOverlap(new CqCapsule(point0, point1, radius), result,layerMask);
                    if (hitNum > 0) // 角度筛选
                    {
                ...
                case ShapeType.Capsule:
                    ...
                    hitNum = Collision.CapsuleOverlap(new CqCapsule(point0, point1, shape.Radius), result,layerMask);
                    break;
                case ShapeType.Ray:
                    ...
                    hitNum = Collision.RayCast(new CqRay(centerPos, dir), rayLen, _raycastHit,layerMask);
                    for (int i = 0; i < hitNum; i++)
                    {
                ...
                case ShapeType.RingFanColumn:
                    ...
                    for (int i = 0; i < hitNum; i++)
                    {
                        if (result[i] is CqCapsuleCollider capsuleCollider)
                        {
                        ...
                        else if (result[i] is CqSphereCollider sphereCollider)
                        {
        ...
        private static bool TryHitColliderByRay(Ray ray, float maxDis, int mask, CqCollider hitCollider)
        {
            var hit = new CqHit();
            return hitCollider.Raycast(ray, maxDis, ref hit);
        ...
        private static int _CapsuleTest(Vector3 centerPos, Vector3 up, float height, float radius, Vector3 direction, float moveDistance, int layerMask)
        {
            ...
            using (ProfilerDefine.CollisionCapsuleCastPMarker.Auto())
            {
                int hitNum = Collision.CapsuleCast(new CqCapsule(point0, point1, radius), direction, moveDistance, _raycastHit,layerMask);
                return hitNum;
            }
        }
        ...
        /// <param name="resultNumLimit">检测结果的数量限制</param>
        /// <returns>检测到的，不重复的Actor对象</returns>
        public static int ContinueCollisionTest(Vector3 StartCenterPos, Vector3 EndCenterPos, Quaternion rot, BoundingShape shape, CqCollider[] result, int layerMask)
        {
            ShapeType type = shape.ShapeType;
            QueryTriggerInteraction flag = QueryTriggerInteraction.Ignore;
        ...
        /// </summary>
        /// <returns></returns>
        public static bool SegmentSegmentPoint2D(Vector3 a, Vector3 b, Vector3 c, Vector3 d, out Vector3 hitPos)
        {
            hitPos = default(Vector3);
            var np = MathUtility.SegmentSegmentNearestPoint(a, b, c, d);
            float distSq = math.distancesq(np.c0, np.c1);
            if (distSq < 1e-4)
            {
                hitPos = np.c0;
                return true;
