...
namespace LCG
{
    public class ABCheck : MonoBehaviour
    {
        // 单例
        private static ABCheck _instance;
        public static ABCheck Instance
        {
            get
            {
                if (null == _instance)
                {
                    GameObject ab = new GameObject("ABHotfix");
                    GameObject.DontDestroyOnLoad(ab);

                    _instance = ab.AddComponent<ABCheck>();
                }
                return _instance;
            }
        }
