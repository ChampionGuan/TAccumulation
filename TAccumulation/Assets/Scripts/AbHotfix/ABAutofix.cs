...
namespace LCG
{
    public class ABAutofix : MonoBehaviour
    {
        // 单例
        private static ABAutofix _instance;
        public static ABAutofix Instance
        {
            get
            {
                if (null == _instance)
                {
                    GameObject ab = new GameObject("ABAutofix");
                    GameObject.DontDestroyOnLoad(ab);

                    _instance = ab.AddComponent<ABAutofix>();
                }
                return _instance;
            }
        }
        ...
        private void CollectRedundantFiles()
        {
            ...
            List<string> imageFilePath = ABHelper.GetAllFilesPathInDir(ABHttpImage.LocalStorgePath);
