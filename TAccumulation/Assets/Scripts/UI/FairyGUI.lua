...
function fgui.register_extension(url, extension)
	...
	if base==CSharp.GComponent then base=FairyGUI.GLuaComponent
		elseif base==CSharp.GLabel then base=FairyGUI.GLuaLabel
			elseif base==CSharp.GButton  then base=FairyGUI.GLuaButton
				elseif base==CSharp.GSlider then base=FairyGUI.GLuaSlider
					elseif base==CSharp.GProgressBar then base=FairyGUI.GLuaProgressBar
						elseif base==CSharp.GComboBox then base=FairyGUI.GLuaComboBox
						else
...
function fgui.extension_class(base)
	...
	o.base = base or CSharp.GComponent
...
function fgui.add_timer(interval, repeatCount, func, obj, param)
	...
	local delegate = fgui_internal.GetDelegate(func, obj, true, CSharp.TimerCallback)
	CSharp.Timers.inst:Add(interval, repeatCount, delegate, callbackParam)
end

function fgui.remove_timer(func, obj)
	local delegate = fgui_internal.GetDelegate(func, obj, false)
	if delegate~=nil then
		CSharp.Timers.inst:Remove(delegate)
	end
end
...
local EventListener_mt = getmetatable(CSharp.EventListener)
...
local EventDispatcher_mt = getmetatable(CSharp.EventDispatcher)
